...
class DeployedQuestionnaireAccessScope extends BaseAccessScope
{
    ...
    private function applyFacultyBasedAccess(Builder $builder, array $rules, $user): void
    {
        ...
        $builder->whereHas('targets', function ($query) use ($facultyId, $programIds) {
            $query->where('faculty_id', $facultyId)
                  ->orWhereIn('program_id', $programIds)
                  ->orWhereHas('semesterCourse', function ($q) use ($facultyId) {
                      $q->whereHas('course', function ($c) use ($facultyId) {
