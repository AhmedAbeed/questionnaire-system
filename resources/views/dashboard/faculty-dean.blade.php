...
<x-layouts.breadcrumbbar
        title="الرئيسية"
        :breadcrumbs="[
            ['name' => 'الرئيسية', 'url' => route('faculty-dean.home'), 'active' => true],
        ]"
    />@endsection
...
@push('scripts')
    <script>
        window.config = {
            statsEndpoint: "{{ route('faculty-dean.dashboard.stats') }}",
            chartDataEndpoint: "{{ route('faculty-dean.dashboard.chart-data') }}"
        };
    </script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.3/dist/chart.umd.min.js"></script>
    <script src="{{ asset('assets/js/pages/faculty-dean.js') }}"></script>
@endpush
