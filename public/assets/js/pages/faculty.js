$(document).ready(() => {
    ...
    // === Config & Endpoints ===
    const scriptTag = document.querySelector('script[src*="faculty.js"]');
    let endpoints = {};
    ...
    const statCards = ['total-faculties', 'total-programs'];
    ...
    const populateStats = data => {
        try {
            $('#total-faculties-value').html(`<h4>${data.total_faculties.value}</h4>`);
            $('#total-faculties-updated').html(`${data.total_faculties.updated}`);
            $('#total-programs-value').html(`<h4>${data.total_programs.value}</h4>`);
            $('#total-programs-updated').html(`${data.total_programs.updated}`);
        } catch (error) {
    ...
    $(document).on('click', '.delete-faculty', function() {
        ...
        Swal.fire({
            title: 'هل أنت متأكد؟',
            text: 'سيتم حذف الكلية وجميع البرامج والمقررات والطلاب والاستبيانات واستجاباتها المرتبطة بها بشكل نهائي ولن تتمكن من استعادتها!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    if (confirmResult.isConfirmed && confirmResult.value === 'موافق') {
                        const $button = $(`.delete-faculty[data-id="${facultyId}"]`);
                        showLoadingState($button);
                        $.ajax({
                            ...ajaxDefaults,
                            url: endpoints.destroyEndpoint.replace(':faculty', facultyId),
                            type: 'DELETE',
                            success: response => {
