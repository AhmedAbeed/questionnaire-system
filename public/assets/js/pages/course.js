$(document).ready(() => {
    // === Config & Endpoints ===
    const scriptTag = document.querySelector('script[src*="course.js"]');
    let endpoints = {};
    ...
    const populateStats = data => {
        try {
            $('#total-courses-value').html(`<h4>${data.total_courses.value}</h4>`);
            $('#total-courses-updated').html(`${data.total_courses.updated}`);
        } catch (error) {
    ...
    $(document).on('click', '.delete-course', function() {
        ...
        Swal.fire({
            title: 'هل أنت متأكد؟',
            text: 'سيتم حذف المقرر وجميع التسجيلات والاستبيانات المرتبطة به بشكل نهائي ولن تتمكن من استعادتها!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    if (confirmResult.isConfirmed && confirmResult.value === 'موافق') {
                        const $button = $(`.delete-course[data-id="${courseId}"]`);
                        showLoadingState($button);
                        $.ajax({
                            ...ajaxDefaults,
                            url: endpoints.destroyEndpoint.replace(':course', courseId),
                            type: 'DELETE',
                            success: response => {
