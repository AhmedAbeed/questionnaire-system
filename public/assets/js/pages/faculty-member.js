$(document).ready(() => {
    ...
    // === Config & Endpoints ===
    const scriptTag = document.querySelector('script[src*="faculty-member.js"]');
    let endpoints = {};
    ...
    const statCards = ['total-faculty-members'];
    ...
    const populateStats = data => {
        try {
            $('#total-faculty-members-value').html(`<h4>${data.total_faculty_members.value}</h4>`);
            $('#total-faculty-members-updated').html(`${data.total_faculty_members.updated}`);
        } catch (error) {
    ...
    // --- Delete Handlers ---
    $(document).on('click', '.delete-faculty-member', function() {
        const facultyMemberId = $(this).data('id');
        Swal.fire({
            title: 'هل أنت متأكد؟',
            text: 'سيتم حذف عضو هيئة التدريس وجميع المقررات والاستبيانات واستجاباتها المرتبطة به بشكل نهائي ولن تتمكن من استعادتها!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    title: 'تأكيد الحذف',
                    text: 'يرجى كتابة "موافق" للتأكيد',
                    input: 'text',
                    inputPlaceholder: 'اكتب: موافق',
                    showCancelButton: true,
                    confirmButtonText: 'تأكيد',
                    cancelButtonText: 'إلغاء',
                    preConfirm: (inputValue) => {
                        if (inputValue !== 'موافق') {
                            Swal.showValidationMessage('يجب كتابة: موافق');
                        }
                        return inputValue;
                    }
                }).then(confirmResult => {
                    if (confirmResult.isConfirmed && confirmResult.value === 'موافق') {
                        const $button = $(`.delete-faculty-member[data-id="${facultyMemberId}"]`);
                        showLoadingState($button);
                        $.ajax({
                            ...ajaxDefaults,
                            url: endpoints.destroyEndpoint.replace(':faculty-member', facultyMemberId),
                            type: 'DELETE',
                            success: response => {
